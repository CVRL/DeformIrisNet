...
from dataset import PairFromBinDatasetBoth, PairExtremeBinDataset
...
def resume(args):
    ...
    # Declare Dataloaders
    train_dataset = PairFromBinDatasetBoth(args.train_bins_path, args.parent_dir, res_mult=args.res_mult)
    ...
    if args.val_bins_path:
        val_dataset = PairFromBinDatasetBoth(args.val_bins_path, args.parent_dir, res_mult=args.res_mult)
    ...
    if args.optim_type == 'adam':
        optimizer = AdamW(deform_net.parameters(), lr = args.lr, amsgrad=True)
...
def findMaxLR(args):
    ...
    if args.model_type == 'denseunet':
        ...
        deform_net = NestedUNetv2(num_classes=1, num_channels=2, width=32, deep_supervision=False)
    
    print(deform_net)    
        
    deform_net = deform_net.to(device)
    
    if not args.super_convergence:
        if args.optim_type == 'adam':
            optimizer = AdamW(deform_net.parameters(), lr=args.lr, amsgrad=True)
        elif args.optim_type == 'sgd':
            optimizer = SGD(deform_net.parameters(), lr=args.lr, momentum=0.9)
        ...
        optimizer = optim.AdamW(deform_net.parameters(), amsgrad=True)
    
    
    criterion = OverallLoss(args, device)
    
    lr_finder = LRFinder(deform_net, optimizer, criterion, device)
    lr_finder.range_test(train_dataloader, end_lr=10, num_iter=1000)
    lr_finder.plot()
...
def train(args):
    ...
    if not args.super_convergence:
        ...
        optimizer = AdamW(deform_net.parameters(), amsgrad=True)
        scheduler = lr_scheduler.OneCycleLR(optimizer, 1.0, epochs=200, steps_per_epoch=len(train_dataloader))
    ...
    for epoch in range(1, args.num_epochs+1):
        ...
        soft_hinge = HingeLossWithSoftLabels(device, label_smoothing=0.2/epoch)
...
def evaluate(cfg, args):
    test_dataset = PairExtremeBinDataset(args.test_bins_path, args.parent_dir)   
...
def evaluate_minmax(cfg, args):
    test_dataset = PairExtremeBinDataset(args.test_bins_path, args.parent_dir, max_pairs_inp = 4)   
    test_dataloader = DataLoader(test_dataset, num_workers=args.num_workers, batch_size=args.batch_size, shuffle=False)
        ...
        for rep in range(args.val_repeats):
            ...
            for batch, data in tqdm(enumerate(test_dataloader), total=len(test_dataloader)):
                ...
                for b in tqdm(range(small_img_t.shape[0]), total=small_img_t.shape[0]):
                    ...
                    #o_pxyrs.append(o_pxyr)
                    #o_ixyrs.append(o_ixyr)
    ...
    for i in tqdm(range(len(s_imgs)), total=len(s_imgs)):
        if s_pxyrs[i][2] > b_pxyrs[i][2]:
            b_im_polar, b_mask_polar = irisRec.cartToPol_prev(b_imgs[i], b_masks[i], b_pxyrs[i], b_ixyrs[i])
            s_displacements = irisRec.findRadiusBiomech(b_pxyrs[i][2], s_pxyrs[i][2], b_ixyrs[i][2], s_ixyrs[i][2])
            s_im_polar, s_mask_polar = irisRec.bioMechCartToPol(s_imgs[i], s_masks[i], s_pxyrs[i], s_ixyrs[i], s_displacements)
            cv2.imwrite('./polar_samples/'+str(i)+'.png', s_im_polar) 
        elif s_pxyrs[i][2] < b_pxyrs[i][2]:
            s_im_polar, s_mask_polar = irisRec.cartToPol_prev(s_imgs[i], s_masks[i], s_pxyrs[i], s_ixyrs[i])
            b_displacements = irisRec.findBioMechDisplacements(s_pxyrs[i][2], b_pxyrs[i][2], s_ixyrs[i][2], b_ixyrs[i][2])
            b_im_polar, b_mask_polar = irisRec.bioMechCartToPol(b_imgs[i], b_masks[i], b_pxyrs[i], b_ixyrs[i], b_displacements)
            cv2.imwrite('./polar_samples/'+str(i)+'.png', b_im_polar)
        else:
            s_im_polar, s_mask_polar = irisRec.cartToPol_prev(s_imgs[i], s_masks[i], s_pxyrs[i], s_ixyrs[i])
            b_im_polar, b_mask_polar = irisRec.cartToPol_prev(b_imgs[i], b_masks[i], b_pxyrs[i], b_ixyrs[i])
        
        s_im_polar_nb, s_mask_polar_nb = irisRec.cartToPol_prev(s_imgs[i], s_masks[i], s_pxyrs[i], s_ixyrs[i])
        b_im_polar_nb, b_mask_polar_nb = irisRec.cartToPol_prev(b_imgs[i], b_masks[i], b_pxyrs[i], b_ixyrs[i])
        o_im_polar, o_mask_polar = irisRec.cartToPol_prev(o_imgs[i], b_masks[i], b_pxyrs[i], b_ixyrs[i])
        ...
        score_deformnet, shift_deformnet = irisRec.matchCodes(code_o_img, code_b_img_nb, b_mask_polar_nb, b_mask_polar_nb)
        ...
        for j in tqdm(range(i+1, len(s_imgs)), total=(len(s_imgs)-i-1)):
            if s_pxyrs[i][2] > b_pxyrs[j][2]:
                s_im_polar, s_mask_polar = irisRec.cartToPol(s_imgs[i], s_masks[i], s_pxyrs[i], s_ixyrs[i])
                s_displacements = irisRec.findBioMechDisplacements(b_pxyrs[j][2], s_pxyrs[i][2], b_ixyrs[j][2], cfg['polar_height'])
                b_im_polar, b_mask_polar = irisRec.bioMechCartToPol(b_imgs[j], b_masks[j], b_pxyrs[j], b_ixyrs[j], s_displacements)
            elif s_pxyrs[i][2] < b_pxyrs[j][2]:
                b_im_polar, b_mask_polar = irisRec.cartToPol(b_imgs[j], b_masks[j], b_pxyrs[j], b_ixyrs[j])
                b_displacements = irisRec.findBioMechDisplacements(s_pxyrs[i][2], b_pxyrs[j][2], s_ixyrs[i][2], cfg['polar_height'])
                s_im_polar, s_mask_polar = irisRec.bioMechCartToPol(s_imgs[i], s_masks[i], s_pxyrs[i], s_ixyrs[i], b_displacements)
            else:
                s_im_polar, s_mask_polar = irisRec.cartToPol(s_imgs[i], s_masks[i], s_pxyrs[i], s_ixyrs[i])
                b_im_polar, b_mask_polar = irisRec.cartToPol(b_imgs[j], b_masks[j], b_pxyrs[j], b_ixyrs[j])
            ...
            o_im_polar, o_mask_polar = irisRec.cartToPol(o_imgs[i], b_masks[j], b_pxyrs[j], b_ixyrs[j])
    ...
    print('Daugman d (Input <-> Target): ', np.mean(ds_ds), '+-', np.std(ds_ds), ', Biomech d (Output <-> Target): ', np.mean(bs_ds), '+-', np.std(bs_ds), ', Deformnet d (Output <-> Target): ', np.mean(ns_ds), '+-', np.std(ns_ds), ', fte: ', fte/total)   
...
def evaluate_biomech_minmax(cfg, args):
    ...
    with torch.no_grad():
        for rep in range(args.val_repeats):
            ...
            for i in tqdm(range(len(s_imgs)), total=len(s_imgs)):
                if s_pxyrs[i][2]/s_ixyrs[i][2] > b_pxyrs[i][2]/b_ixyrs[i][2]:
                    b_im_polar, b_mask_polar = irisRec.cartToPol_prev(b_imgs[i], b_masks[i], b_pxyrs[i], b_ixyrs[i])
                    s_displacements = irisRec.findRadiusBiomech(b_pxyrs[i][2], s_pxyrs[i][2], b_ixyrs[i][2], s_ixyrs[i][2])
                    s_im_polar, s_mask_polar = irisRec.bioMechCartToPol(s_imgs[i], s_masks[i], s_pxyrs[i], s_ixyrs[i], s_displacements)
                    #cv2.imwrite('./polar_samples/'+str(i)+'.png', np.uint8(np.concatenate([s_im_polar, s_mask_polar], axis=0))) 
                elif s_pxyrs[i][2]/s_ixyrs[i][2] < b_pxyrs[i][2]/b_ixyrs[i][2]:
                    s_im_polar, s_mask_polar = irisRec.cartToPol_prev(s_imgs[i], s_masks[i], s_pxyrs[i], s_ixyrs[i])
                    b_displacements = irisRec.findRadiusBiomech(s_pxyrs[i][2], b_pxyrs[i][2], s_ixyrs[i][2], b_ixyrs[i][2])
                    b_im_polar, b_mask_polar = irisRec.bioMechCartToPol(b_imgs[i], b_masks[i], b_pxyrs[i], b_ixyrs[i], b_displacements)
                    #cv2.imwrite('./polar_samples/'+str(i)+'.png', np.uint8(np.concatenate([b_im_polar, b_mask_polar], axis=0)))
                else:
                    s_im_polar, s_mask_polar = irisRec.cartToPol_prev(s_imgs[i], s_masks[i], s_pxyrs[i], s_ixyrs[i])
                    b_im_polar, b_mask_polar = irisRec.cartToPol_prev(b_imgs[i], b_masks[i], b_pxyrs[i], b_ixyrs[i])
                ...
                code_s_img_nb = irisRec.extractCode(s_im_polar_nb)                
                code_b_img_nb = irisRec.extractCode(b_im_polar_nb)
                raw_code_b_img_nb = irisRec.extractRawCode(b_im_polar_nb)
                raw_code_b_img_nb -= raw_code_b_img_nb.min()
                raw_code_b_img_nb /= raw_code_b_img_nb.max()
                
                for i in range(7):
                    cv2.imwrite('bsif_samples/BSIF_code_' + str(i) +'.png', np.uint8(code_b_img_nb[:, :, i]*255))
                    cv2.imwrite('bsif_samples/Continuous_code_'+str(i)+'.png', np.uint8(raw_code_b_img_nb[:, :, i]*255))
                exit()
                ...
                if score_daugman_genuine is not None and score_biomech_genuine is not None and not math.isnan(score_daugman_genuine) and not math.isnan(score_biomech_genuine): 
                ...
                for j in tqdm(range(i+1, len(s_imgs)), total=(len(s_imgs)-i-1)):
                    id2 = pair_ids[j]
                    if id1 != id2:
                        if s_pxyrs[i][2]/s_ixyrs[i][2] > b_pxyrs[j][2]/b_ixyrs[j][2]:
                            b_im_polar, b_mask_polar = irisRec.cartToPol(b_imgs[j], b_masks[j], b_pxyrs[j], b_ixyrs[j])
                            s_displacements = irisRec.findRadiusBiomech(b_pxyrs[j][2], s_pxyrs[i][2], b_ixyrs[j][2], s_ixyrs[i][2])
                            s_im_polar, s_mask_polar = irisRec.bioMechCartToPol(s_imgs[i], s_masks[i], s_pxyrs[i], s_ixyrs[i], s_displacements)
                            #cv2.imwrite('./polar_samples/'+str(i)+'_'+str(j)+'.png', np.uint8(np.concatenate([s_im_polar, s_mask_polar], axis=0))) 
                        elif s_pxyrs[i][2]/s_ixyrs[i][2] < b_pxyrs[j][2]/b_ixyrs[j][2]:
                            s_im_polar, s_mask_polar = irisRec.cartToPol(s_imgs[i], s_masks[i], s_pxyrs[i], s_ixyrs[i])
                            ...
                            #cv2.imwrite('./polar_samples/'+str(i)+'_'+str(j)+'.png',  np.uint8(np.concatenate([b_im_polar, b_mask_polar], axis=0))) 
                        else:
                            s_im_polar, s_mask_polar = irisRec.cartToPol(s_imgs[i], s_masks[i], s_pxyrs[i], s_ixyrs[i])
                            b_im_polar, b_mask_polar = irisRec.cartToPol(b_imgs[j], b_masks[j], b_pxyrs[j], b_ixyrs[j])
                        ...
                        score_daugman_imposter, shift_daugman_imposter = irisRec.matchCodes(code_s_img_nb, code_b_img_nb, s_mask_polar_nb, b_mask_polar_nb)
                        score_biomech_imposter, shift_biomech_imposter = irisRec.matchCodes(code_s_img_biomech, code_b_img_biomech, s_mask_polar, b_mask_polar)
                        
                        if score_daugman_imposter is not None and score_biomech_imposter is not None and not math.isnan(score_daugman_imposter) and not math.isnan(score_biomech_imposter):
                            daugman_scores_imposter.append(score_daugman_imposter)
                            biomech_scores_imposter.append(score_biomech_imposter)
                            total += 1
                        else:
                            fte += 1
                            total += 1
            ...
            bs_d = abs(bsg_mean - bsi_mean)/math.sqrt(0.5 * (bsg_var + bsi_var))
            bs_ds.append(bs_d)
            print('Daugman d (Input <-> Target): ', ds_d, ', Biomech d (Output <-> Target): ', bs_d, ', fte: ', fte/total)   
    
    print('Daugman d (Input <-> Target): ', np.mean(ds_ds), '+-', np.std(ds_ds), ', Biomech d (Output <-> Target): ', np.mean(bs_ds), '+-', np.std(bs_ds), ', fte: ', fte/total)   
...
def sample(args):
    ...
    test_dataset = PairExtremeBinDataset(args.test_bins_path, args.parent_dir, max_pairs_inp=2)
    test_dataloader = DataLoader(test_dataset, num_workers=args.num_workers, batch_size=args.batch_size, shuffle=False)
...
def checkSIFLayer(cfg, args):
    ...
    test_dataset = PairExtremeBinDataset(args.test_bins_path, args.parent_dir, max_pairs_inp=4, input_size=(240,320))   
    test_dataloader = DataLoader(test_dataset, num_workers=args.num_workers, batch_size=args.batch_size, shuffle=False)
...
def makevideo(args, input_size=(240,320)):
    test_video_keys = PairExtremeBinDataset(args.test_bins_path, args.parent_dir).bins.keys()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--super_convergence', action='store_true')
    parser.add_argument('--find_max_lr', action='store_true')
    parser.add_argument('--evaluate_only_biomech', action='store_true')
    ...
    if args.train:
        ...
        evaluate_minmax(get_cfg(args.cfg_path), args)
    elif args.sample:
        sample(args)  
