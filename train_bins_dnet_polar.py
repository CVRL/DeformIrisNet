...
from dataset import PairFromBinDataset, AllPairsDataset, PairMinMaxBinDataset, PairFromBinDatasetPolar, PairMinMaxBinDatasetPolar
from network import SIFLayerPolar, TVLoss, VGGPerceptualLoss, LPIPSLoss, DenseUNetPolar, L1LossWithSoftLabels, BCELogitsLossWithSoftLabels, CrossEntropyLoss2d
...
def resume(args):
    checkpoint_dir = './dnet_polar_cp_' + datetime.datetime.now().strftime('%Y%m%d%H%M%S%f') + '/'
    softmax = nn.LogSoftmax(dim=1)
    # Declare Dataloaders
    train_dataset = PairFromBinDatasetPolar(args.train_bins_path, args.parent_dir, input_size=(240,320))
    ...
    if args.val_bins_path:
        val_dataset = PairFromBinDatasetPolar(args.val_bins_path, args.parent_dir, input_size=(240,320))
    ...
    if args.use_sif_loss:
        ...
        sifLossModel.eval()
        '''
        sifLossModel = SIFLayerPolar(fiter_mat = filter_mat, device=device).to(device)
        sifLossModel.eval()
        
    if args.optim_type == 'adam':
        optimizer = Adam(deform_net.parameters(), lr = args.lr, betas=(0.9, 0.99), eps=1e-8, amsgrad=True, weight_decay=1e-5)
    ...
    for epoch in range(1, args.num_epochs+1):
        ...
        for batch, data in enumerate(train_dataloader):
            ...
            small_imgs_polar = data['small_img']
            small_masks_polar = data['small_mask']
            
            big_imgs_polar = data['big_img']
            big_masks_polar = data['big_mask']
            ...
            inp = Variable(torch.cat([small_imgs_polar, small_masks_polar], dim=1)).to(device)
            
            tar = Variable(big_imgs_polar).to(device)
            tar_mask = Variable(data['big_mask']).to(device)
            tar_mask_norm = (tar_mask+1)/2

            all_out = deform_net(inp, big_img_pxyr[:,2], big_img_ixyr[:,2])
            
            out = nn.Tanh()(all_out[:, 0, :, :])
            out_mask = all_out[:, 1:3, :, :] 
            ...
            if args.use_mask_loss:
                #mask_loss = nn.CrossEntropyLoss(label_smoothing=0.1)(out_mask, tar_mask_norm.requires_grad_(False))      
                mask_loss = CrossEntropyLoss2d()(out_mask, tar_mask_norm.requires_grad_(False))     
                epoch_mask_loss.append(args.beta * mask_loss.item())
            else:
                mask_loss = 0.
            
            if args.use_sif_loss:
                sif_out = sifLossModel(out_norm * 255)
                sif_tar = sifLossModel(tar_norm * 255)
                tar_mask_polar_rep = torch.cat([tar_mask_norm]*7, dim=1).requires_grad_(False)
                minus_one = -1 * torch.ones(sif_tar.shape).to(device)
                sif_tar_binary = torch.where(sif_tar > 0, one, zero).requires_grad_(False)
...
def train(args):
    ...
    softmax = nn.LogSoftmax(dim=1)
    # Declare Dataloaders
    train_dataset = PairFromBinDatasetPolar(args.train_bins_path, args.parent_dir, input_size=(240,320), polar_width=512, polar_height=64)
    train_dataloader = DataLoader(train_dataset, num_workers=args.num_workers, batch_size=args.batch_size, shuffle=True)
    
    if args.val_bins_path:
        val_dataset = PairFromBinDatasetPolar(args.val_bins_path, args.parent_dir, input_size=(240,320), polar_width=512, polar_height=64)
        val_dataloader = DataLoader(val_dataset, num_workers=args.num_workers, batch_size=args.batch_size, shuffle=False)
    else:
        val_dataloader = None
    ...
    deform_net = DenseUNetPolar(num_classes=3, num_channels=2, width=4, image_shape=(64, 512))
    deform_net.apply(init_weights)
    ...
    if args.use_sif_loss:
        ...
        sifLossModel.eval()
        '''
        sifLossModel = SIFLayerPolar(filter_mat = filter_mat, device=device).to(device)
        sifLossModel.eval()
    ...
    for epoch in range(1, args.num_epochs+1):
        ...
        for batch, data in enumerate(train_dataloader):
            ...
            big_img_pxyr = Variable(data['big_img_pxyr']).to(device)
            big_img_ixyr = Variable(data['big_img_ixyr']).to(device)
            ...
            out = nn.Tanh()(all_out[:, 0, :, :]).reshape(inp.shape[0], 1, inp.shape[2], inp.shape[3])
            out_mask = all_out[:, 1:3, :, :] 
            ...
            if args.use_mask_loss:
                #mask_loss = nn.CrossEntropyLoss(label_smoothing=0.1)(out_mask, tar_mask_norm.requires_grad_(False))      
                mask_loss = CrossEntropyLoss2d()(out_mask, tar_mask_norm[:,0].long().requires_grad_(False))        
                epoch_mask_loss.append(args.beta * mask_loss.item())
            else:
                mask_loss = 0.
            ...
            if args.use_sif_loss:
                sif_out = sifLossModel(out_norm * 255)
                sif_tar = sifLossModel(tar_norm * 255)
                tar_mask_polar_rep = torch.cat([tar_mask_norm]*7, dim=1).requires_grad_(False).to(device)
                one = torch.ones(sif_tar.shape).to(device)
                minus_one = -1 * torch.ones(sif_tar.shape).to(device)
                zero = torch.zeros(sif_tar.shape).to(device)
                sif_tar_binary = torch.where(sif_tar > 0, one, zero).requires_grad_(False)
                sif_tar_binary_masked = torch.clamp((sif_tar_binary * tar_mask_polar_rep).requires_grad_(False), 0, 1).float()
                sif_tar_hinge_masked = torch.where(sif_tar_binary_masked < 0.5, one, minus_one)
                sif_out_hinge_masked = (nn.Tanh()(sif_out) * tar_mask_polar_rep).requires_grad_(True)
                ...
                if args.sif_hinge:
                    ...
                    #sif_loss = BCELogitsLossWithSoftLabels(device)(nn.Flatten()(sif_out), nn.Flatten()(sif_tar_binary), nn.Flatten()(tar_mask_polar_rep))
                    sif_loss = nn.BCEWithLogitsLoss(pos_weight=nn.Flatten()(tar_mask_polar_rep))(nn.Flatten()(sif_out), nn.Flatten()(sif_tar_binary))
                else:
                    sif_tar_l1_masked = torch.where(sif_tar_binary_masked > 0.5, one, minus_one)
                    sif_loss = L1LossWithSoftLabels(device)(sif_out_hinge_masked, sif_tar_l1_masked)
                epoch_sif_loss.append(args.gamma * sif_loss.item())
                sif_out_binary = torch.where(sif_out.clone().detach().requires_grad_(False) > 0, one, zero)
                out_mask_prob = nn.LogSoftmax(dim=1)(out_mask.clone().detach().requires_grad_(False))
                out_mask_binary = torch.argmax(out_mask_prob, dim=1)
                out_mask_rep = torch.cat([out_mask_binary.reshape(-1, 1, out_mask.shape[2], out_mask.shape[3])]*7, dim=1)
                andMasks = out_mask_rep * tar_mask_polar_rep
                epoch_sif_diff.append(torch.mean((torch.abs(sif_out_binary - sif_tar_binary) * andMasks).flatten()) * 100)
                epoch_sif_diff_nomask.append(torch.mean((torch.abs(sif_out_binary - sif_tar_binary) * tar_mask_polar_rep).flatten()) * 100)
            else:
                sif_loss = 0.
            ...
            if vb_batch % args.log_batch == 0 and vb_count == 0:
                ...
                if args.use_sif_loss:
                    ...
                    sif_bit_average = sum(epoch_sif_diff) / len(epoch_sif_diff)
                    loss_string += ", SIF bit diff: {sif_bit}%".format(sif_bit = sif_bit_average)
                    sif_bit_nomask_average = sum(epoch_sif_diff_nomask) / len(epoch_sif_diff_nomask)
                    loss_string += ", SIF bit diff no mask: {sif_bit}%".format(sif_bit = sif_bit_nomask_average)
                ...
                for i in range(args.val_repeats):
                    ...
                    for batch, data in enumerate(val_dataloader):
                        ...
                        if args.use_sif_loss:
                            ...
                            out_mask_rep = torch.cat([out_mask_binary.reshape(-1, 1, out_mask.shape[2], out_mask.shape[3])]*7, dim=1)
                            andMasks = out_mask_rep * tar_mask_polar_rep 
                            val_bit_diff.append(torch.mean((torch.abs(sif_out_binary - sif_tar_binary) * andMasks).flatten()) * 100)
                            val_bit_diff_nomask.append(torch.mean((torch.abs(sif_out_binary - sif_tar_binary) * tar_mask_polar_rep).flatten()) * 100)
                        else:
                            sif_loss = 0.
                    ...
                    val_loss_average += (sum(val_epoch_loss) / len(val_epoch_loss))
                    val_bit_diff_average += (sum(val_bit_diff) / len(val_bit_diff))
                    val_bit_diff_nomask_average += (sum(val_bit_diff_nomask) / len(val_bit_diff_nomask))
                
                val_loss_average /= args.val_repeats
                val_bit_diff_average /= args.val_repeats
                val_bit_diff_nomask_average /= args.val_repeats
                print("Val loss: {aver}, Val bit diff: {bit_diff}, Val bit diff no mask: {bit_diff_nm}, (epoch: {epoch})".format(aver = val_loss_average, bit_diff = val_bit_diff_average, bit_diff_nm = val_bit_diff_nomask_average, epoch = epoch))
                if args.log_file is not None:
                    with open(args.log_file, 'a') as f:
                        f.write("Val loss: {aver}, Val bit diff: {bit_diff}, Val bit diff no mask: {bit_diff_nm}, (epoch: {epoch})".format(aver = val_loss_average, bit_diff = val_bit_diff_average, bit_diff_nm = val_bit_diff_nomask_average, epoch = epoch) + '\n')
                
                if val_bit_diff_nomask_average < best_val_bit_diff:
                    best_val_bit_diff = val_bit_diff_nomask_average
                    if not os.path.exists(checkpoint_dir):
                        os.mkdir(checkpoint_dir)
                    filename = checkpoint_dir + "{epoch:04}-val_bit_diff-{bit_diff}.pth".format(epoch = epoch, bit_diff=val_bit_diff_nomask_average)
                    torch.save(deform_net, filename)
    return True
